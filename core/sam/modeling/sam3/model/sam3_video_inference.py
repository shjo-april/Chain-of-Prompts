...
from core.sam.modeling.sam3 import perflib
from .model_misc import clone_output_wrapper
from core.sam.utils.box_ops import box_xywh_to_cxcywh, box_xyxy_to_xywh
from core.sam.utils.data_misc import BatchedDatapoint, convert_my_tensors, FindStage
from core.sam.modeling.sam3.model.geometry_encoders import Prompt
from core.sam.modeling.sam3.model.io_utils import IMAGE_EXTS, load_resource_as_video_frames
from core.sam.modeling.sam3.model.sam3_tracker_utils import fill_holes_in_mask_scores
from core.sam.modeling.sam3.model.sam3_video_base import MaskletConfirmationStatus, Sam3VideoBase
from core.sam.modeling.sam3.perflib.compile import compile_wrapper, shape_logging_wrapper
from core.sam.utils.masks_ops import masks_to_boxes as perf_masks_to_boxes
from core.sam.utils.utils import copy_data_to_device
...
class Sam3VideoInference(Sam3VideoBase):
    ...
    def _warm_up_vg_propagation(self, inference_state, start_frame_idx=0):
        ...
        for i, thresh in enumerate(new_det_score_thresh_list):
            ...
            for num_objects in num_objects_list:
                logging.info(f"{i+1}/{num_rounds} warming up model compilation")
                self.add_prompt(
    ...
    def warm_up_compilation(self):
        ...
        logging.info("Warm-up compilation completed.")
    ...
    def add_prompt(
        ...
        """
        logging.debug("Running add_prompt on frame %d", frame_idx)
...
class Sam3VideoInferenceWithInstanceInteractivity(Sam3VideoInference):
    ...
    def propagate_in_video(
        ...
        if propagation_type == "propagation_full":
            logging.debug(f"Running full VG propagation (reverse={reverse}).")
