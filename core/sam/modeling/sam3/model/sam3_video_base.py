...
from core.sam.utils.box_ops import fast_diag_box_iou
from core.sam.utils.data_misc import BatchedDatapoint
from core.sam.modeling.sam3.model.sam3_tracker_utils import fill_holes_in_mask_scores, mask_to_box
from core.sam.utils.masks_ops import rle_encode, mask_iou
...
class Sam3VideoBase(nn.Module):
    def __init__(
        ...
        logging.info(f"setting {max_num_objects=} and {num_obj_for_compile=}")
    ...
    def _load_checkpoint(self, ckpt_path: str, strict: bool = True):
        ...
        if len(missing_keys) > 0 or len(unexpected_keys) > 0:
            logging.warning(f"Loaded ckpt with {missing_keys=}, {unexpected_keys=}")
        else:
            logging.info("Loaded ckpt successfully without missing or unexpected keys")
