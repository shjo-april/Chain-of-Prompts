...
try:
    ...
    logging.debug(
        "cc_torch not found. Falling back to CPU connected components (via skimage). "
        "Consider installing for better GPU performance. Command line:"
...
def connected_components(input_tensor: torch.Tensor):
    ...
    # CPU fallback (also used on CUDA when cc_torch is not available)
